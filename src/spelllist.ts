...
export async function renderSpellList(source: string, el: HTMLElement, _ctx?: MarkdownPostProcessorContext) {
  ...
  if (!names.length) {
    el.createEl("div", { text: "No spell names provided." });
    return;
  }
...
async function renderSingleSpell(host: HTMLElement, baseUrl: string, name: string) {
  ...
  try {
    ...
    console.error("Failed to render spell in list", { name, id, error: e });
    renderCollapsible(host, displayNameFromSlug(id) + " (Error)", "Error loading this spell");
  }
}
...
function renderCollapsible(el: HTMLElement, title: string, html: string) {
  const contentDivId = `spelllist-content-${Math.random().toString(36).slice(2, 11)}`;
  const arrowId = `spelllist-arrow-${Math.random().toString(36).slice(2, 11)}`;
  el.innerHTML = `
