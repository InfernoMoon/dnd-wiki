...
import { renderSingleSpell } from "./spell";
...
export async function renderSpellList(source: string, el: HTMLElement, _ctx?: MarkdownPostProcessorContext) {
  ...
  for (const line of lines) {
    ...
    if (mClass) {
      ...
      if (/^all$/i.test(raw)) {
        ...
        classDirective = parts.map(p => nameToSlug(p)).filter(Boolean);
      }
      continue;
    }
    ...
    if (mSchool) {
      ...
      if (/^all$/i.test(raw)) {
        ...
        const parts = raw.split(',').map(s => s.trim()).filter(Boolean);
        schoolDirective = parts.map(p => nameToSlug(p)).filter(Boolean);
      }
      continue;
    }
  }
  ...
  const toNames = (docs: (Document | null)[]) => docs.filter((d): d is Document => !!d).map(d => extractNames(d));
